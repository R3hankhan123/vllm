...
def enumerate_resources(resource_map, mask=None, allowed=None):
    ...
    for cpu in resource_map["cpus"]:
        ...
        if (
            ...
            socket = _int(cpu.get("socket", 0))
            if lscpu["sockets"].get(socket, None) is None:
                lscpu["sockets"][socket] = [cpu]
            else:
                lscpu["sockets"][socket].append(cpu)
            book = _int(cpu.get("book", 0))
            if lscpu["books"].get(book, None) is None:
                lscpu["books"][book] = [cpu]
            else:
                lscpu["books"][book].append(cpu)
            drawer = _int(cpu.get("drawer", 0))
            if lscpu["drawers"].get(drawer, None) is None:
                lscpu["drawers"][drawer] = [cpu]
            else:
                lscpu["drawers"][drawer].append(cpu)
    return lscpu
...
def create_omp_places(resources, strategy, smt=True):
    ...
    elif strategy == "drawers":
        for value in resources["drawers"].values():
            omp_places.append(produce_cpu_sublist(value, smt))
    elif strategy == "books":
        for value in resources["books"].values():
            omp_places.append(produce_cpu_sublist(value, smt))
    elif strategy == "sockets":
        for value in resources["sockets"].values():
            omp_places.append(produce_cpu_sublist(value, smt))
    elif strategy == "nodes":
